...
def list_project(user_info):
    ...
    return jsonify(dict(code=0, data=SzrResponse.model_to_list(result), msg="操作成功"))


@pr.route("/insert", methods=["POST"])
@permission(szr.config.get("MANAGER"))
...
def query_project(user_info):
    ...
    if err is not None:
        return jsonify(dict(code=110, data=result, msg=err))
    result.update({"project": SzrResponse.model_to_dict(data), "roles": SzrResponse.model_to_list(roles),
                   "test_case": tree})
